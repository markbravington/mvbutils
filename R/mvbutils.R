...
#    copy.ns.objects( cq( fix.list, maintained.packages), pkgname)
  ...
  if( F) sapply( c(
      ...
      'character.only', 'check.for.tracees', 'chtml', 'cleanup', 'Cloaders',
      'compress.lazyload', 'configure', 'CONTENTS', 'DAY', 'detectCores', 'dir.', 'DLL',
      'dll.paths', 'dlls', 'doc', 'docType', 'dont.check.visibility', 'dynamic.help', 'env',
      'ewhere', 'EXAMPLES', 'exclude.data', 'exclude.funs', 'exec', 'execute.First',
      'execute.Last', 'export.me', 'exports.have.changed', 'fixing', 'force.all.docs',
      'funmat', 'funs', 'generics', 'getDoParName', 'highlight', 'html', 'i',
      'importing_Clink', 'INDEX', 'inst', 'INSTALL', 'installed', 'ipath', 'is.Rd2',
      'last.R.major', 'last.warning', 'latex', 'level', 'libs', 'LICENCE', 'LICENSE', 'lldir',
      'load.from', 'm', 'makeR3', 'man', 'MD5', 'meta', 'mfilename', 'mfiles',
      'minstrl', 'mods.in.packages', 'mods.in.tasks', 'MONTH', 'MP', 'mtrace',
      'mvb.session.info', 'mvbutils', 'my.reps', 'n', 'name', 'name.load.from', 'NAMESPACE', 'NEWS',
      'orig.path', 'outdir', 'override.answer', 'package', 'pkg', 'prepend', 'prune',
      'Rd.version', 'Rdir', 'ROOT', 'Rrebver', 'run_postcopy_hooks', 's', 'sep',
      'sourcedir', 'spath', 'src', 'stopCluster', 'tarball', 'task', 'task.path',
      'task.tree', 'tasks', 'tests', 'textcolor', 'to', 'trace.was.on', 'tracees',
      'tricky_dicky', 'type.load.from', 'USAGE', 'useDynLib', 'Version', 'vignettes', 'vscode',
      'w', 'what', 'where', 'x', 'y', 'Y', 'YEAR', 'YES'
    ), assign, value= function(...) "CRANtidote", envir=environment( sys.function()))
...
# detectCores etc via make_CRANtidote to subvert spurious NOTE. Therefore, best to cause
# a crash if parallel can't be loaded

stopifnot( re_bloody_quire( 'parallel'))
  avail <- detectCores()
  if( is.na( avail)){
    avail <- 1L
  }
...
It works by creating a dummy for each of those variables in the namespace environment, specifically a function that returns the string "CRANtidote" (since as-CRAN checks look not just for variables, but for functions). Sigh.

Unwanted side-effects are unlikely, but possible: here's two examples. First, when I initially tried this with 'mvbutils' itself, it noticed that '.onAttach' was mentioned directly somewhere, and "helpfully" tried to add that--- causing failure on 'attach()', since R tried to run a non-function '.onAttach'. So '.onAttach' is now specifically excluded to avoid this, and will need tweaking manually in your code. There might be other special things only created/needed after '.onLoad' that need similar attention.

Second, if you 'requireNamespace' a package (it's OK if that's in Suggests) but don't refer to its members by name (e.g. 'otherpak::funcaroo', which R says you're Supposed To but IMO that's silly as standard advice; it makes code harder to follow, and the code is all inside your own package, so _you_ will know whether there's any conflicting definition of 'funcaroo'; if there is, do use double-colons obvs) , then its members will already have been created as fake functions by 'make_CRANtidote' (and they need to ). So you gotta double-colon them, or something like that. Sigh.
...
A character vector of class 'cat', which will display nicely. If the p
...
function(
  ...
  PARALLEL <- PARALLEL && 
      ...
      !is.null( foreach::getDoParName()) # checks if cluster is regjoed or wotevs
...
return( eval.parent( mc))
    ...
    # This piece of idiocy is the antidote to the CRANtidote of stupid "no global" false-pos check warnings
    if( exists( 'foreach', asNamespace( 'mvbutils'))){
      `%dopar%` <- foreach::'%dopar%'
