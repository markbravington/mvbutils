...
All pre-existing files in the "man", "src", "tests", "exec", "demo", "inst", and "R" sourcedirectories of the source-package directory will be removed (unless you have some 'mlazy' objects; see below). If--- but this is deprecated--- a file ".Rbuildignore" is present in the task package, then it's copied twice to the package directory, also under the name ".Rinstignore" (so that it's also excluded from Windows/Mac binaries--- crazy R shit!). If not but there is an object '<pkg>.Rbuildignore' (the preferred way; it should be a character vector), then that's used (and is automatically augmented to exclude some task-package housekeeping files). To exclude files that would otherwise be copied, i.e. those in "inst/demo/src/data" folders, create a character vector of regexes called '<<pkg>>.file.exclude.regexes'; any file matching any of these won't be copied. (I'm not sure I've ever used this feature, but I have now gotten the allegedly-deprecated-by-me method using a ".Rbuildignore" _file_ to work--- the key was the ".Rinstignore".)
